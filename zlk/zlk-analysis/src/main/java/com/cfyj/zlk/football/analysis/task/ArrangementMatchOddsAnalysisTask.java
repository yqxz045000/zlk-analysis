...
public class ArrangementMatchOddsAnalysisTask {
	...
	public void arrangementMatchOddsAnalysisTask() {
		if(!flag) {
			return ;
		}
		flag = false;
	...
	private void arrangementMatchOddsResult(List<Match> mlist) {
		...
		if (mlist != null && mlist.size() != 0) {
			for (ListIterator<Match> it = mlist.listIterator(); it.hasNext();) {
				...
				Match match = it.next();
				if(match.getFullScore().length()>4) {
					continue;
				}
...
//			mor.setOfailOddsKl(oddskl2[2]);
	...
	private Double[] getOddsRate(String odds[]) {
		...
		result[0] = ArithUtil.mul(ArithOddsUtil.arithWinRate(odds[0], odds[1], odds[2]), "100"); // 主胜
		result[1] = ArithUtil.mul(ArithOddsUtil.arithWinRate(odds[1], odds[0], odds[2]), "100");// 平
		result[2] = ArithUtil.mul(ArithOddsUtil.arithWinRate(odds[2], odds[0], odds[1]), "100");// 客胜
