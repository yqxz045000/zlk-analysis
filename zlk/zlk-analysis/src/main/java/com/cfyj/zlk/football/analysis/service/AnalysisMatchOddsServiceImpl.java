...
import com.cfyj.zlk.football.utils.LineChartUtil;
...
public class AnalysisMatchOddsServiceImpl implements AnalysisServivce {
	...
	@Override
	public Map<String, Object> getLineCart(long qtid,String companyId) {

		MatchOddsResult matchOddsResult =matchOddsResultMapper.findByQtidAndCompanyId(qtid,companyId);
		
		String[] strA = matchOddsResult.getAllOdds().split(";");
		String[] oddsAllA = matchOddsResult.getAllOdds().split(";");
		List<MatchOddsResultExportDomain> datas = new ArrayList<>();
		for (int i = oddsAllA.length; i < 0; i--) {
			if (StringUtils.isNotBlank(oddsAllA[i])) {
				MatchOddsResultExportDomain me = new MatchOddsResultExportDomain();
				String oddsStr[] = oddsAllA[i].split(",");
				Double[] rateA = getOddsRate(oddsStr);
		...
		return LineChartUtil.pointUselessAnalyze(datas);
	}
